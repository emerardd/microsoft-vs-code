...
- **统一游戏时钟**：移动、射击、装填、Buff、敌人和碰撞均以 60 Hz 固定时间步推进；单帧最多补算 250 ms，超长卡顿统一丢弃超额时间。斜向移动归一化；护盾接触伤害每 100 ms 触发 60 点。
...
- **反射 API**：每层增加一次侧壁反弹，最多 2 层；基础武器也会获得斜射弹。
...
扩展在完成升级选择后保存下一波的检查点。重新加载或重启恢复已打开的游戏标签时，会暂停在该波起点；中途弹幕和敌人位置不会保存。新开一局或死亡会清除检查点。普通隐藏/显示仍保留当前战局。详细设计见 [优化说明](docs/optimization-notes.md)。
